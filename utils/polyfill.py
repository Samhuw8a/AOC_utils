...
def _prod(iterable: Iterable[Any], start: int = 1) -> int:
    """
...
def _pow(base: int, exp: int, mod: Optional[int] = None) -> IntOrFloat:
    """
    ...
    if exp >= 0:
        ...
        return real_pow(base, exp, mod)  # type: ignore

    # Polyfill for Python < 3.8
    return real_pow(_modinv(base, mod), -exp, mod)  # type: ignore
...
if sys.version_info >= (3, 9):
    ...
    from math import prod, comb, perm, isqrt  # type: ignore

    gcd = _gcd
    lcm = _lcm
    pow = _pow  # type: ignore
else:
    gcd = _gcd
    lcm = _lcm
    prod = _prod
